...
# ADDITION TO CODE
# Generate all 1 million possibilities
all_numbers = []
all_questions = []
all_expected = []
...
print('Vectorization...')
all_x = np.zeros((len(all_questions), MAXLEN, len(chars)), dtype=np.bool)
all_y = np.zeros((len(all_questions), DIGITS + 1, len(chars)), dtype=np.bool)
for i, sentence in enumerate(all_questions):
    all_x[i] = ctable.encode(sentence, MAXLEN)
for i, sentence in enumerate(all_expected):
    all_y[i] = ctable.encode(sentence, DIGITS + 1)
